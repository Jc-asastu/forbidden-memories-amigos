...
window.renderArena=()=>{
 ...
 $('arena-room-actions').replaceChildren();if(r){const self=r.players.find(p=>p.id===n.id);const ready=button(self?.ready?'Cancelar listo':'Estoy listo','arena-button',()=>action('ready',{ready:!self?.ready}));ready.id='arena-ready';ready.disabled=r.state!=='waiting'||state.game.running;$('arena-room-actions').append(ready);if(r.hostId===n.id){const start=button(r.state==='preparing'?'Sincronizando…':'Iniciar duelo','arena-button',()=>action('start-match'));start.id='arena-start';start.disabled=r.state!=='waiting'||r.players.length!==2||!r.players.every(p=>p.ready);$('arena-room-actions').append(start);}if(r.state==='playing')$('arena-room-actions').append(node('p','El duelo está abierto.','subtle'));}
